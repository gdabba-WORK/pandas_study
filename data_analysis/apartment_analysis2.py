...
df_2013_2015 = pd.read_csv('../data/전국 평균 분양가격(2013년 9월부터 2015년 9월까지).csv',
                           ...
                           header=0,
                           engine='python')
df_2015_2019 = pd.read_csv('../data/전국 전체 분양가격(2015_2019).csv',
                           encoding='utf-8',
                           engine='python')
...
print("df_2013_2015.columns = year")
print("df_2013_2015 :")
print(df_2013_2015, '\n\n')

# 통계정보 제거
df_2013_2015 = df_2013_2015.drop(df_2013_2015.index[[0, 1, 2, 10, 12, 22]])
print("df_2013_2015 = df_2013_2015.drop(df_2013_2015.index[[0, 1, 2, 10, 12, 22]])")
print("df_2013_2015 :")
print(df_2013_2015, '\n\n')

df_2013_2015.loc[4, '구분'] = ''
df_2013_2015.loc[14, '구분'] = ''
print("df_2013_2015.loc[4, \'구분\'] = \'\'",
      "df_2013_2015.loc[14, \'구분\'] = \'\'", sep='\n')
print("df_2013_2015 :")
print(df_2013_2015, '\n\n')
...
print("df_2013_2015['구분'] = df_2013_2015['구분'].fillna('')",
      "df_2013_2015.시군구 = df_2013_2015.시군구.fillna('')", sep='\n')
print("df_2013_2015 :")
print(df_2013_2015, '\n\n')

df_2013_2015['지역명'] = df_2013_2015.구분 + df_2013_2015.시군구
print("df_2013_2015['지역명'] = df_2013_2015.구분 + df_2013_2015.시군구")
print("df_2013_2015 :")
print(df_2013_2015, '\n\n')
df_2013_2015 = df_2013_2015.drop(['구분', '시군구'], axis=1)
print("df_2013_2015 = df_2013_2015.drop(['구분', '시군구'], axis=1)")
print("df_2013_2015 :")
print(df_2013_2015, '\n\n')

# 행을 열로 전환
melt_columns = df_2013_2015.columns.copy()
print("melt_columns = df_2013_2015.columns.copy()")
print("melt_columns, type(melt_columns) :")
print(melt_columns, type(melt_columns), sep='\n', end='\n\n')

# df_2013_2015 = pd.melt(df_2013_2015, id_vars=['지역명'],
#                        value_vars=['2013 12', '2014 1', '2014 2', '2014 3', '2014 4', '2014 5', '2014 6',
#                                    '2014 7', '2014 8', '2014 9', '2014 10', '2014 11', '2014 12', '2015 1',
#                                    '2015 2', '2015 3', '2015 4', '2015 5', '2015 6', '2015 7', '2015 8',
#                                    '2015 9'])

# column label을 list로 변경
melt_columns = melt_columns[:len(melt_columns) - 1].tolist()
print("melt_columns = melt_columns[:len(melt_columns)-1].tolist()")
print("melt_columns :", '\n', melt_columns, '\n\n')

df_2013_2015 = pd.melt(df_2013_2015, id_vars=['지역명'],
                       value_vars=melt_columns)
print("df_2013_2015 = pd.melt(df_2013_2015, id_vars=['지역명'],",
      "value_vars=melt_columns)")
print(df_2013_2015.head(), '\n\n')

# 지역명, 0, value => '지역명', '기간', '분양가'로 열이름 별경
df_2013_2015.columns = ['지역명', '기간', '분양가']
print("df_2013_2015.columns = ['지역명', '기간', '분양가']")
print(df_2013_2015.head(), '\n\n')
...
df_2015_2019 = df_2015_2019.drop(columns=df_2015_2019.columns[0])
...
# 컬럼명 맞추기
print("df_2013_2015.columns :"), print(df_2013_2015.columns, '\n\n')
print("df_2015_2019.columns :"), print(df_2015_2019.columns, '\n\n')
...
df_2015_2019_prepare.columns = total_columns

print("df_2013_2015_prepare.head() :")
print(df_2013_2015_prepare.head(), '\n\n')
print("df_2015_2019_prepare.head() :")
print(df_2015_2019_prepare.head(), '\n\n')
print("df_2013_2015_prepare.shape :")
print(df_2013_2015_prepare.shape, '\n\n')
print("df_2015_2019_prepare.shape :")
print(df_2015_2019_prepare.shape, '\n\n')

# 결합하기
df_2013_2019 = pd.concat([df_2013_2015_prepare, df_2015_2019_prepare])
print("df_2013_2019 = pd.concat([df_2013_2015_prepare, df_2015_2019_prepare])")
print("df_2013_2019.shape :")
print(df_2013_2019.shape, '\n\n')

df_year_mean = df_2013_2019.groupby(['연도'])['평당분양가격'].mean()
print("df_year_mean = df_2013_2019.groupby(['연도'])['평당분양가격'].mean()")
print(df_year_mean, '\n\n')
...
plt.subplots_adjust(left=0.1, right=0.9, bottom=0.1, top=0.87, wspace=0.5, hspace=0.5)  # 여백 지정
...
ax1.set_title('연도별 평균 평당분양가격 - pandas-bar')
ax2.set_title('연도별 평균 평당분양가격 - seaborn-bar')
ax3.set_title('연도별 평균 평당분양가격 - pandas-boxplot')
ax4.set_title('대구 연도별 평균 평당분양가격 - seaborn-boxplot')
